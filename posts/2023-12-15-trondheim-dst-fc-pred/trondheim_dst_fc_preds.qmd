---
title: "Trondheim 20k Skiathlon Predictions"
author: "Joran Elias"
date: "2023-12-15"
categories: [world cup,distance,skiathlon,mass start,predictions,trondheim]
draft: false
---
...
### Predictions
First skiathlon...the women's ranking are still liking Frida, despite the 
poor results she's had so far. It's always a tricky balance.
...
tech <- "Skiathlon"
mens_preds <- 
  readr::read_csv(file = '2023-12-15_men_fc_dst_trondheim.csv',
                  show_col_types = FALSE)
womens_preds <- 
  readr::read_csv(file = '2023-12-15_wom_fc_dst_trondheim.csv',
                  show_col_types = FALSE)
```

#### Men's 20k Skiathlon Predictions
```{r tbl-men}
mens_preds |> 
  select(-compid) |> 
  gt() |> 
...
#### Women's 20k Skiathlon Predictions
```{r tbl-wom}
womens_preds |> 
  select(-compid) |> 
  gt() |> 
...
dst_res <- v_dst_maj_int |> 
  filter(eventid %in% c(19277,19276)) |> 
  collect()
...
As I suspected, it way overrated Frida, but overall we're still doing quite well.
