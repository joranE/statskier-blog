...
And.....things go back to a weekly schedule and I promptly forget that the sprint races are on Friday, not Saturday. Ah well.

```{r pred-data}
location <- "Oberhof"
tech <- "Classic"
mens_preds <- 
  readr::read_csv(file = '2024-01-19_men_cl_spr_oberhof.csv',
                  show_col_types = FALSE)
womens_preds <- 
  readr::read_csv(file = '2024-01-19_wom_cl_spr_oberhof.csv',
                  show_col_types = FALSE)
```
...
spr_res <- v_spr_maj_int |> 
  filter(eventid %in% c(19520,19521)) |> 
  collect()
...
Nearly identical this time. Oh well.
